...
namespace CUtility 
{
    ...
    static _vec3 Picking_Terrain(LPDIRECT3DDEVICE9 m_pGraphicDevice, HWND hWnd, CTerrainVillage* pObj)
    {
        if (pObj == nullptr)
        {
        ...
        for (_ulong i = 0; i < VILLAGE_VTXCNTZ - 1; ++i)
        {
            for (_ulong j = 0; j < VILLAGE_VTXCNTX - 1; ++j)
            {
                _ulong dwIndex = i * VILLAGE_VTXCNTX + j;

                dwVtxNumber[0] = dwIndex + VILLAGE_VTXCNTX;
                dwVtxNumber[1] = dwIndex + VILLAGE_VTXCNTX + 1;
